...
Usually set by the server using `Set-Cookie` response header, normally has a session identifier. Subsequent outgoing requests automatically have the previously received cookie sent in `Cookie` request header, this way the server knows who is making the request. 
...
There are several 'attributes/options' that can be set for any cookie when they are set. They are listed after the `key=value` and they too are delimited using `;`

```
document.cookie = "user=John; path=/; expires=Tue, 19 Jan 2038 03:14:07 GMT"
```

> So cookies are delimited using `;` and the attributes of a given cookie are also delimited using `;`. <br>How does the browser know what is a cookie and what is an option? May be options are pre-set constants and hence can be recognized that way.
...
> Note that an attribute need not necessarily be a key-value pair.<br>These attributes are sent in the `Set-Cookie` response header, so don't expect them to be present in subsequent outgoing request.
...
This can be avoided to some extent using the cookie option `samesite`. If `samesite=strict` is set then siteB just cannot have the siteA's cookie sent. If `samesite=lax` is set then siteB can have siteA's cookie sent only if its trying to access siteA using a GET and if its a top-level option (such as a hyperlink being opened from the browser) and not from a form-submission, etc.
...
>  I accessed cricinfo, and saw these cookies (belonging to domain '*.fls.doubleclick.net') in Chrome dev tools. When I cleared them, change the Chrome settings to not allow Third-party cookies and reloaded the page, these cookies were not there. So these are likely examples of Third-party cookies, they are not placed by the domain of the page that was actually loaded in the browser.

![image-20230806121630957](assets/image-20230806121630957.png)

###### Default setting in various browsers

Third-party cookies can be blocked by certain browsers. Safari does not allow them at all (this however can be enabled using Safari Settings, the 'all cookies' there basically means 'all third-party cookies').

Chromeby default blocks third-party cookies only in Incognito mode but this can be changed through Settings > Privacy and Security > Cookies and other site data.

| Safari                                                       | Chrome                                                       |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| ![image-20230806121326770](assets/image-20230806121326770.png) | ![image-20230806121210076](assets/image-20230806121210076.png) |
