...
A cookie is called `third-party` if it’s placed by a domain other than the page the user is visiting. They are traditionally used for tracking and ads services (i.e. when a page from one site has a an ad presented by JS from another site), due to their nature.
