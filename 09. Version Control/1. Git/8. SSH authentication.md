When interacting with a remote, there are various ways in which authentication can be done. While the traditional way is to do `password based authantication` where the username and password needs to be entered in the shell console everytime a remote operation is done, `SSH authentication` is a more secure mechanism. When you connect to a remote server via SSH, you authenticate using a private key file on your local machine.
