...
> Without RVM, Ruby just gets installed in system folder, i.e. `/usr/bin/ruby`. So at a time only one Ruby version can be available and every gem that it has installed becomes available to all projects. Even though different projects would rather have different versions of a given gem. (I think that is how it happens without RVM.)

`which ruby` tells the installation location of the currently active Ruby version.
...
```
`sSL` flag tells to download using SSL(v1?). `-s` makes bash read commands from stdin, and accept positional parameters nonetheless. `--` lets bash treat everything which follows as positional parameters instead of options. ([link](https://stackoverflow.com/a/51854728/1135417)). `stable` here is something that is inside the script that resides at https://get.rvm.io.
...
`gems` has all the installed gemsets. Gemsets are arranged as `RubyVersion@GemsetName` and these gemsets internally have the precise gems they need to have. <br><br> `gemsets` is another folder but it seems to be less relevant. It seems to be having one folder each for all the Ruby implementations (i.e. `ruby`, `jruby`, etc.) and each of these folders have their respective installed versions and they in turn something called `global.gems` file. <br><br>

| `rubies`  | `gems` | `gemsets`
| ------------- | ------------- | -------------
| ![](assets/2.RVM-7215200f.png)  | ![](assets/2.RVM-d826b57d.png) | ![](assets/2.RVM-fc27056f.png)

> The Ruby implementations managed by RVM are placed in folders like `/Users/sou543/.rvm/rubies/ruby-2.5.3/bin/ruby`.
...
```
gem install GemName (Installs the gem in the current Rubyset/repository)
...
RVM is 'hands off' any system ruby that is installed. To be able to 'use' the system ruby you can tell RVM to undo the environment changes that it has applied, as follows.
```
rvm use system
```
Doing a `which ruby` thereafter gives the location of system installed ruby like `/usr/bin/ruby` and also `rvm list` does not show any of its Rubies as the currently used version. Thereafter a RVM managed version can be switched back to using something like `rvm use default` and then `which ruby` is also reverted to a RVM location.

`ruby -v` gives the current Ruby version that is active. If a RVM Ruby version is being used it gives that, and if the system Ruby version is being used it gives that.
> So at a given point the entire sytem has only one Ruby version active (and not different Ruby versions in different folders or shell sessions)?
...
```
ruby -v (Ruby version being used in current Shell session, intelligent enough to take care of both system Ruby and RVM)
