...
```
echo 'eval "$(/opt/homebrew/bin/brew shellenv)"' >> /Users/anand/.zprofile
...
As packages are installed, they get placed in `/homebrew/Cellar/` directory. The precise version of a given package is called `keg`. Loosely speaking a package is called `rack`, its corresponding diretory has one or more versioned kegs. Cellar directory has various racks placed in it.
