...
The idea with **CLAUDE.md** file is that its persistent memory automatically loaded in every session/CW. There are various levels of this file which then dictate if that file is loaded across all projects, or just particular ones, and even in some specific folders in projects.
...
When there are plenty of content that still needs to be loaded from start, you can split them into multiple files and place in `.claude/rules' folder.
