...
> SwiftUI can be tricky. There are so many different ways that a view's appearance can be modified and also in the sense that can be so many attributes for the same kind of thing. Take color for example. A view can have an accent color, foreground color, background color, preferred color scheme.
