...
##### Using xcrun swiftc 'dump-ast' option (~2023)

As of 2025 emit-sytntax is obsolete and dump-ast can be used instead.

```swift
xcrun swiftc -dump-ast One.swift
```

It generates this

```swift
(source_file "One.swift"
  (func_decl range=[One.swift:1:1 - line:1:30] "one()" interface_type="() -> Int" access=internal result="Int" thrown_type="<null>"
    (parameter_list range=[One.swift:1:9 - line:1:10])
    (brace_stmt range=[One.swift:1:19 - line:1:30]
      (return_stmt range=[One.swift:1:21 - line:1:28]
        (integer_literal_expr type="Int" location=One.swift:1:28 range=[One.swift:1:28 - line:1:28] value="1" builtin_initializer="Swift.(file).Int.init(_builtinIntegerLiteral:)" initializer="**NULL**")))))%  
```
...
> Note that the version is important, as its usually tied to the Xcode version. What works now will likely not work with a later major Xcode version. List of swift-syntax versions ([link](https://github.com/swiftlang/swift-syntax/releases))
...
##### Using SwiftParser to print syntax tree (2023)
...
##### Using SwiftParser to print syntax tree (2025)

```swift
private static func testSyntaxParser() {    
    let codeString  =
      """
      import Foundation
    ...
    let rootNode: SourceFileSyntax = Parser.parse(source: codeString)
    print(rootNode)
    print("--------")
    recursivePrint(node: Syntax(rootNode), indent: 0)
}

private static func recursivePrint(node: Syntax, indent: Int) {
    let indentString = String(repeating: "  ", count: indent)
    let nodeKind = node.kind

    print("*******\(indentString)\(nodeKind) - \n\(indentString)\(node.description)")
