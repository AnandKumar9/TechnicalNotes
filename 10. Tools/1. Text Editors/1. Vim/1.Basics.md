...
> Any of above can also be prefixed with a number to do it that many times. For instance `3b` will go to the start of 3rd word from the current cursor position.

`#` Previous occurrence  of current word <br>
`*` Next occurrence of current word <br>

`%` Locate nearest parentheses (`(` `{` `[`) and go to its counterpart <br>
...
> So `54G` will move the cursor to line no. 54
...
Wrap selection in backticks ``` <br>
Wrap selection in three backticks <br>
Toggle case <br>
Replace all occurences in seletion with wrapped backticks <br>
