...
Use Homebrew to install two packages - node (for running JS outside browser), watchman (a package from Facebook to watch files and record when they change, used to reload simulator as source code files change)
...
The xcworkspace itself can be run from Xcode too, but the server has to be running for UI to show up (that can be done using the usual `npx expo start`).
