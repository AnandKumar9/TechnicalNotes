...
Ar runtime, RN creates the native Android and iOS views. The UI components that correspond to these native platform views that can get rendered are called `Native Components`, and such UI compnents that come bundled with RN are called `Core Components`. 
...
`Props` (i.e. properties) that can components be initialized with, which then allow some attribute of the UI to be modified.
