...
> The significance of things in `{{<SOMETHING>}}` is that it has a valid JS coming it, which can then be run from native with dynamic values injected to variables over there at runtime.

##### Step 2: Convery the JSON to a fully ready UIView
