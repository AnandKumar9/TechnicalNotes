...
`fileprivate(set)` `private(set)` `internal(set)` `public internal(set)`
...
`fileprivate(set)`<br>`private(set)`<br>`internal(set)`<br>`public fileprivate(set) var name: String`  - This implies that the property is accessible in public scope but settable only in fileprivate scope.<br>`public internal(set)` - This implies the property is accessible in public scope but settable only in internal scope.<br>
