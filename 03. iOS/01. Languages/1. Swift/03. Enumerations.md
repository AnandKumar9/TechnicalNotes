...
There are two kinds of enums - `frozen`, and `non frozen`. <br>An enum declared as `@frozen` (i.e. `@frozen enum Enum1 {}`) (or with no declaration as frozen is the default) is an indication to complier that this enum is not likely to have new cases going forward. Whereas `non frozen` enums say to compiler that it is indeed likely to get new cases later on, and any switch on it should have a `default` case handling too even if it already has all the known cases listed in the switch.. 

> As of 2020, all enums are frozen by default and only internal iOS APIs can be non frozen. However its kind of tricky to tell which private enum is nonfrozen as the interface does not have any annotation/directive. And the only way to know it is if you have a switch on it and it does not have a default case.

`@unknown default` is needed when there is a switch on a non frozen enum. Switches on non frozen enums should have a `default` or `@unknown default` handling. If it isn't there, a warning will be shown. (`stackView.axis`, i.e. `NSLayoutConstraint.Axis` is actually a non frozen enum).

![](assets/Misc-81abe000.png)

It can then be handled using either a `default` or `unknown default`, both will not generate a warning for non frozen enum if the switch is currently exhaustive. However if new cases are later added in the enum, in the switch while `default` will stay silent, `unknown default` will instead thankfully give a warning saying there are unhandled cases.

> @frozen appears to be applicable for structs as well. For example, in SwiftUI `@frozen struct Text`. 

> As of 2023, this seems to have evolved more. Apparently, there can be `@nonfrozen` enums in libraries and frameworks if they are built with Library Evolution Support ([link](https://forums.swift.org/t/non-frozen-enums-in-swift/62484/2)). 👈 Try this and see how it can materially make a difference for the library's consumer.
