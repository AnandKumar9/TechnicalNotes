...
Closures can capture and store references to any constants and variables from the context in which they are defined. This is known as _"closing over those constants and variables"_.

Technically, closures are 'reference types'.
...
By default, a closure passed as an argument to a function cannot be stored in some context outside of that function's implementation (if its in a different file). In other words, by default closure parameters are `non-escaping`. <br>
...
So the thing with non-escaping closures (i.e. the default type for closures in functions) is that they should not be saved and referenced later after the function has returned. Yet the function could be doing something inside which requires the closure to be escaping even if paratically the closure will get invoked before the function returns. For such cases, `withoutActuallyEscaping()` function (introduced in Swift 3.1) can be used. [(Reference)](https://developer.apple.com/documentation/swift/2827967-withoutactuallyescaping) 

It takes in a non-escaping closures and passes an escapable copy of it to the second closure where that copy can be invoked. Essentially, its creating a temporary escaping copy of the closure.

> If however anything is done where the closure is somehow getting retained beyond the time the function returns, there will be unpredictable behavior including runtime error (which is actually worse than the compiler error that happens if trying to use a non-escaping closure as an escaping closure).

For example, 

| This will not compile as `script` is not marked `@escaping`  | This will compile as `withoutActuallyEscaping()` is being used and compiler has been told to ignore the closure inside, but then at runtime there will be an error |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| ![image-20250211123533478](./assets/image-20250211123533478.png) | ![image-20250211123610889](./assets/image-20250211123610889.png) |

A more real example from the documentation. The LHS will not compile as `lazy` collection's `filter()` invocation expects an escaping closure. But the RHS will compile, and also run well as `lazy.filter()` is guranteed to return beforethe function can return.

| This will not compile                                        | This will compile and run well                               |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| ![image-20250211125002765](./assets/image-20250211125002765.png) | ![image-20250211125014356](./assets/image-20250211125014356.png) |
