...
`withoutActuallyEscaping(_:do)` - Swift 3.1 introduced [this](https://developer.apple.com/documentation/swift/2827967-withoutactuallyescaping) function which takes in two non-escaping closures and allows one of the closures to be escaping while the other one is still running. So it temporarily allows one non-escaping closure to be escaping.
