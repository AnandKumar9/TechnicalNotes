...
> The gist of the entire idea - That a fXCOunction's execution can be suspended indefinitely with an easy syntax.
...
```
testMethod2 above (☝️) declares itself to be a function that will respond in an _async_ manner and hence anyone calling it should call it using the appropriate syntax.
...
The below snippets are not doing literally the same thing. The former function returns something right away, but the latter will not hand the execution back to caller right away (it will cause the calling function's execution to suspend). The latter version however does ensure that something is mandatorily 'returned' on completion which couldn't be enforced by compiler in the former version, as its syntactically fine to not call the completion handler at all (FWIW using completion handlers there is an example of what is termed as 'unstructured concurrency').
...
Concurrent binding overcomes one shortcoming of typical use of async await (i.e. sequential binding). Using it, when an async function is called the caller function does not suspend and yet when a point comes that something returned by the async function actually needs to be used the caller function does suspend.
...
When an async function is called, the function hands over the control of the thread to the system (right away when called using 'await' and a little later when called using 'async let/var') and lets it do other work that may be important to it. Once that work finishes, the system hands the thread back to the function at its suspension point. Normal functions don't have a way to suspend in the middle of execution. **So to put it very simply, that's what async/await mainly is - allowing system more frequent access to threads that would otherwise have sat idle for some time in a non async-await world.** Note though that both async and await are just an indication to runtime that the system that it can suspend these functions if it wants to, it may after all chose not to.
...
#### Examples of converting completion handler based API to Swift Concurrency based API

| Completion Handler based code                                | Swift concurrency based code                                 |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| ![image-20250316162617541](./assets/image-20250316162617541.png) | ![image-20250316162633625](./assets/image-20250316162633625.png) |
...
In its very early stages, programming languages had `jump` like statements which allowed the code to jump to just about anywhere. Later on control-flow statements like loop were introduced which restricted whereall a codebase can jump and also became more defined on where the symbols of a given block can be used. This more defined programming is what is known as `Structured Programming` in the world of programming languages.
