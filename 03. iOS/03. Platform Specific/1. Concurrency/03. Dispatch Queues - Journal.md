...
> Got from ChatGPT 👇. Its essentialy a way to ensure thread safety.

A dispatch semaphore in iOS (part of Grand Central Dispatch, or GCD) is a synchronization tool used to control access to a resource across multiple threads. It works like a counter-based lock. You initialize it with a value representing how many threads are allowed to access the resource concurrently. Threads then wait for the semaphore before proceeding, and signal it when they're done.
