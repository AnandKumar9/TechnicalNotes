...
Basically, several libraries can be combined into one. In debug config its linked during runtime, and in release condig its built dring compilation. So faster build times in debug, and faster launch time in release. In release config, the binary is actually slightly larger  but avoids the overhead of loading dynamic links at runtime.
...
A debug build is unoptimized, where either the clang debug format is `-O0` or the swiftc debug format is `-Onone`. A release build uses optimizations.  Xcode represents debug builds with a new build setting `IS_UNOPTIMIZED_BUILD`.
...
| If not using mergeable lib                                   | If using merged lib                                          |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
...
WWDC 2023: Meet mergeable libraries ([Video](https://developer.apple.com/videos/play/wwdc2023/10268/#:~:text=Mergeable%20libraries%20offer%20size%2C%20build,by%20using%20the%20automatic%20configuration.)) <br>Configuring your project to use mergeable libraries ([Developer documentation](https://developer.apple.com/documentation/xcode/configuring-your-project-to-use-mergeable-libraries)) <br>Understanding Mergeable Libraries ([article](https://www.polpiella.dev/understanding-mergeable-libraries)) <br>