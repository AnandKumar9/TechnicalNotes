...
Even with one core machines concurrency existed as it was possible to time share the core so often that it appeared as if concurrency was being achieved. However as the number of processor cores per chip and the clock speed of CPU (processor) started increasing, the ability to write more sophisticated concurrent code also became more relevant.
