...
If a response has to be provided back to JavaScript, it can be done using the `WKScriptMessageHandlerWithReply` protocol ([reference]()).
...
When the rule list is provided to WebKit, it converts the rules into an efficient bytecode format and its supposed to be very performant (as of ~2017). The fully list of supported triggers and actions is probably [this](https://developer.apple.com/documentation/safariservices/creating_a_content_blocker).
