...
A `UITabBarController` is composed of views that the tab bar controller manages directly and views that are managed by content view controllers that are provided. The interface consists of a tab bar and the content area. Each content view controller manages a distinct view hierarchy.
...
##### Should only be used as allowed
...
##### Tabs can be rearranged after rendering
...
The `moreViewControllers` allows to rearrange tabs. If there are certain tabs that should not be allowed to be rearranged, they should not be included in the `customizableViewControllers` property. View controllers not in this property are not displayed in the the configuration screen and cannot be removed from the tab bar if they are already there.
