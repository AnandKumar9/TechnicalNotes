...
**Scenes** are different regions (windows) that are there in an app. An app can have multiple scenes. On iOS they have just one scene. but on macOS and iPadOS apps can have multiple scenes (i.e. windows). On iOS scenes take up the full screen, on iPadOS scenes need not take full screen and the system decides how much real estate it takes, on macOS scenes can easily have a real estate whose size can be set by the user.
