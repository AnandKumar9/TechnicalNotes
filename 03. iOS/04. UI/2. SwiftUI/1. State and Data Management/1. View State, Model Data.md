> @State <br>@Binding <br>@ObservableObject, @Published, and @ObservedObject <br>@StateObject <br>
...
When a property in a `View` is qualified with `Binding` property wrapper, that property can have its value being set somewhere outside the View and yet any change to the value can trigger a view update.

Also, the outside place where the value is exists should be declared as a `State` property in a `View`, with the binding passed as the state's projected value (i.e. `$`).
