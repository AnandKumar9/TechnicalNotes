...
The entire idea behind static analysis is that it can try and detecy some possible issues in the code without compiling it. For example, something looking like an infinite loop. The kind of things it can checks keeps evolving with every Xcode version and can be selectively chosen through Build Settings. Also, most of these new checks are added by open-source contributions made to Apple's Clang compiler.
