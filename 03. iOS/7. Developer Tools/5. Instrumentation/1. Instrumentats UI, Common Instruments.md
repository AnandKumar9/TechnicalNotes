...
There is an `Immediate mode` and a `Last x seconds mode` while profiling. The former is the defaul mode and the latter can be activated by doing an option click on the Record button. Not too sure, but I think the latter is easier on the system.
