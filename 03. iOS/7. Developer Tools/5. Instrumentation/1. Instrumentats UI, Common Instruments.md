...
For any thread, the `Heaviest Stack Trace` is shown on the right bottom pane. This is the stack for the set of functions that were called most often (i.e. the functions that ran the most on that thread?) during the profile time.
...
While looking at source code, its also possible to look at the 'Disassembly' code.

Option to view disassembly | Source code and disassembly side-by-side
--- | ---
![](assets/Misc-10.59.16-1.png) | ![](assets/Misc-10.59.16-2.png)
...
While running Instruments from Xcode with a `Cmd + I`,if option is pressed in the modal to choose the profiling template the button changes to 'Profile' thereby allowing to start  profiling right away.

#### Best practices while profiling with Instruments

> These are recommended but not mandatory (👇). Even if they are not done, Instruments can still give valuable insight depending on what the goal is.
...
`Control + swipe` allows to select a specific time period in the Time Profiler instrument temlate (potentially in other instruments too?).

As per 'WWDC 2015: Profiling in Depth', Time Profiler is capable of saying how much time is being spent in various functions but not how many times a function has been called in a given time.
