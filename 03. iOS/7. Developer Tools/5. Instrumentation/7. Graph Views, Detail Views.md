...
The kind of graph depends on both the schema type as well as the column from which data is being sourced.
...
If the column being targeted has a 'magnitude', then a bar graph such as this is drawn. If however the column being targeted has a state and the state does not inherently have a magnitude, a different representation (with rounded rectangles) gets used.
