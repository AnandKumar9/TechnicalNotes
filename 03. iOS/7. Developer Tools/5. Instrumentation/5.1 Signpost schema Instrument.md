...
Any instrument package needs to have a schema defined. For creating signposts based instrument, a default schema named `os-signpost-interval-schema` exists which should be used (it can be inserted using a built-in code snippet). Using `<os-signpost-interval-schema>` both defines a schema and automatically generates an appropriate modeler. <br>
...
`plot-template` is a dynamic way of creating graphs. It looks at the table, at the column that was passed in `<instance-by>` element and for each unique value of this column, it creates plot. `<label-format>` element allows to create format title for this plot.

> An example of an instrpg xml that uses `plot-template` and `aggregation` for a signpost schema. From 'WWDC 2018: Creating custom instruments 30:00'.

plot-template spec | aggregation spec
--- | ---
![](assets/Misc-9.56.59-1.png) | ![](assets/Misc-9.56.59-2.png)

##### Signpost related fields available in Instruments

When Instruments record signpost data, all the explicitly specified fields (including the format string) as well as some more more implicit fields too are available for display.

![](assets/Misc-9.40.28.png)

##### Misc.

[Official documentation for `os-signpost-interval-schema`](https://help.apple.com/instruments/developer/mac/10.0/#/dev536412616) <br>
...
[Martin Todorov article on managing hudge instrpkg xmls](https://trycombine.com/posts/xcode-powerups/) (yet to try this) <br>
