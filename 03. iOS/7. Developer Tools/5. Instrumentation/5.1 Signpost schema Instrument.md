...
`plot-template` is dynamic way of creating graphs. It looks at the table, at the column that was passed in `<instance-by>` element and for each unique value of this column, it creates plot. `<label-format>` element allows to create format title for this plot.
