...
`````bash
swift package show-dependencies <OPTIONALLY: --format json>
`````

Can neatly show the dependency graph in shell itself, and if specified also as a nice JSON. When outputted to a JSON, it has more details like the file system locations of the dependencies.

> Show it for Mastodon's app
