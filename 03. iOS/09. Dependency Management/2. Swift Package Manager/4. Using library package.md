...
Add Package Dependency from Target | Select Package
--- | ---
![](assets/Misc-1a85a75a.png) | ![](assets/Misc-a9ac4bdf.png)

Its also possible to add a package through `Project Settings > Package Dependencies`. The Package Dependencies added do show up there too though. While adding from there, it also asks if the lib should also be added to any of the existing targets in the workspace (this actually happens when adding from elsewhere too).

| 1. Add package from Project > Target Dependencies            | 2. Asks if lib should also be added to any of the targets    |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| ![image-20240317172742384](assets/image-20240317172742384.png) | ![image-20240328135341758](assets/image-20240328135341758.png) |
...
#### Packages shown in File Navigator of a workspace

The packages shown in file navigator of a workspace are all the packages used in the workspace, not just the ones explicity added to the workspace from 'Project Settings > Package Dependencies', but even the ones used by any SPM library used by the project. 

So if that SPM library is added as a 'Target Dependency' to any of the targets, then that SPM library will not show up as the project's Package Dependency in its project settings but all the packages used by it will show up regardless in the 'File Navigator > Package Dependencies'.

> Can see this happening in [Mastodon's codebase](https://github.com/mastodon/mastodon-ios.git).
