Kotlin's collection interfaces. ([Reference](https://kotlinlang.org/docs/collections-overview.html#collection-types))
![](assets/Misc-097d9a86.png)
...
##### Essential functions (`forEach`, `filter`, `map`)

```
peopleAges.forEach { print("${it.key} is ${it.value}, ") }
var mappedList = peopleAges.map { "${it.key} is ${it.value}" }
val filteredNames = peopleAges.filter { it.key.length < 4 }
```

Collection transformation functions ([Reference](https://kotlinlang.org/docs/collection-transformations.html))
